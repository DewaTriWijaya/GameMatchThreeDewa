...
public class BoardManager : MonoBehaviour
{
    ...
    private IEnumerator DestroyedAndFillTiles(List<TileController> destroyedTiles, System.Action onCompleted)
    {
        ...
        float spawnHeigh = endPosition.y + tilePrefab.GetComponent<SpriteRenderer>().size.y + offsetTile.y;
        ...
        foreach (TileController tile in destroyedTiles)
        {
            ...
            tile.transform.position = new Vector2(tile.transform.position.x, spawnHeigh);
    ...
    public Vector2 GetIndexPosition(Vector2Int index)
    {
        Vector2 tileSize = tilePrefab.GetComponent<SpriteRenderer>().size;
        return new Vector2(
            startPosition.x + ((tileSize.x + offsetTile.x) * index.x),
